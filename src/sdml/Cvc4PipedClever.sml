...
structure Cvc4Piped :> SOLVER_INTERFACE_CVC4_PIPED = struct
    ...
    fun computeDepth (CVC4{history, ...}) =
          (* foldr: go in reverse, since history is newest-first *)
          List.foldr (fn (Push, n) => n + 1
                       | (Pop, n) => n - 1
                       | (_, n) => n)
                     0
                     history
    ...
    fun CHECK message (cvc4 as CVC4{history, ...}) =
        let val depthFromHistory = computeDepth cvc4
        in
            if !currDepth = depthFromHistory then
                ()
            else
                (print ("CHECK " ^ message ^ ": CVC4 off-kilter: "
                        ^ "depthFromHistory = " ^ Int.toString depthFromHistory ^ ",\n"
                        ^ "but !currDepth = " ^ Int.toString (!currDepth) ^ ")\n"
                        ^ "History is:\n" ^ historyToString (List.rev history) ^ "\n");
                 raise Option)
        end
    ...
    *)
    ...
    fun cvc4_transact_session d inputLine = (* inputLine: do not include ";\n" *)
       (transactionCount := !transactionCount + 1;
        let val session = getCurrSession()
            ...
            val _ = wasteTime 0
        in
            Piper.echo (fn () => "%%% " ^ Int.toString (!currDepth) ^ " / " ^ Int.toString d ^ "\n");
    ...
    fun eternal_cvc4_transact cvc4 command =
       let val result = 
                   cvc4_transact_session (computeDepth cvc4) (commandToString command)
           val _ =
                    case command of
                         Push => (currDepth := !currDepth + 1)
                                  
                       | Pop => (currDepth := !currDepth - 1)
                       | _ => ()
       in
           result
       end
...
(*
   ...
   (* replay (cvc4', cvc4):
    ...
    fun replay (cvc4' as CVC4{history= history', ...},
        ...
        let val cvc4'_depth = computeDepth cvc4'
(*            val _ = print ("cvc4'_depth = " ^ Int.toString cvc4'_depth ^ "\n")
            val _ = print ("!currDepth = " ^ Int.toString (!currDepth) ^ "\n") *)
            val _ = Piper.echo (fn () => "% replay (depth "
                                         ^ Int.toString cvc4'_depth ^ " to "
                                         ^ Int.toString (computeDepth cvc4) ^ ")\n")
            val _ = CHECK "replay.getCurrent()" (getCurrent())
            val _ = CHECK "replay.cvc4'" cvc4'
...
*)            
            ...
            val entryDepth = !currDepth
            val _ = if entryDepth <> cvc4'_depth then
                        (print ("REPLAY WARNING: entryDepth "
                                ^ Int.toString entryDepth
                                ^ " <> " ^ "depth' " ^ Int.toString cvc4'_depth
                                ^ "\n")(*; raise Option*))
                    else ()
            ...
            fun redo commands =
                let fun glob [] = []
                      | glob [c] = [c]
                      | glob (Assert ass1 :: rest) = (print "A"; Assert ass1 :: (glob rest))
    ...
    structure Context = struct
        ...
        fun fromSession session =
           (Piper.transact_session session {input="PUSH;\n", outputSync = isSuffixList prompt};
            let val cvc4 = CVC4{history= [Push],
                                cookie= Cookie.fresh()}
            in
          ...
          X.App(f, X.Tuple [X.Literal (sortname, string1), X.Literal (_, string2)]) =>
              if sortname = X.getIntSort() then
                  let val fString = IndexSym.toShortString f
                      ...
                      fun return n =
                          (print "!"; X.Literal (sortname, Int.toString n))
                  in
                      case fString of
                          "+" => return (num1 + num2)
                        | "-" => return (num1 - num2)
...
(* Start a new context *)
  ...
  fun hasAssert (cvc4 as CVC4{history, ...}) string =
      List.exists (fn Assert h => h = string | _ => false) history

   fun assertCoreString cvc4 string =
       if true andalso (hasAssert cvc4 string) then
          ...
          let val cvc4 = reaffirm cvc4
              ...
              val cvc4PUSH  =
                    if true orelse occasional() then
                        #1 (recorded_cvc4_transact cvc4 Push)
                    else
                         cvc4
              val _ = CHECK "assertCoreString 3" cvc4PUSH
