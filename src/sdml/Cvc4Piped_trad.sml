...
structure Cvc4Piped :> SOLVER_INTERFACE_CVC4_PIPED = struct
    ...
    *)
    ...
    fun eternal_cvc4_transact_list cvc4 commands =
        cvc4_transact_session (Separate.list "; " (List.map commandToString commands))
...
*)
                ...
                in
                    List.app (fn command => ((*print ("-redo- " ^ c2str command ^ "\n"); *)transact cvc4 command))
                             (glob commands)
                end
...
(*            val _ = print ("recorded_cvc4_transact: adding \"" ^ inputLine ^ "\" to history\n")
          ...
          val cvc4 = CVC4{history= inputLine :: history, cookie= Cookie.fresh()}
      in
          (cvc4, result)
      end
...
(*            val _ = print ("recorded_cvc4_transact: adding \"" ^ inputLine ^ "\" to history\n")
          val _ = print ("previous history: \"" ^ Separate.list ";\\n " history ^ "\"\n") *)
          val cvc4 = CVC4{history= recorded1 :: history, cookie= Cookie.fresh()}
      in
          (cvc4, result)
      end
 ...
 *)
   ...
   fun declare (cvc4 as CVC4{history= history, cookie= cookie}, (a, sort)) =
       case sort of
           X.Base sortname =>
